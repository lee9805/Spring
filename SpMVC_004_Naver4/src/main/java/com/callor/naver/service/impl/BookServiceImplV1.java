...
public class BookServiceImplV1 implements BookService{
	...
	/*
	 * GenericDao를 상속받아 BookDao 를 만들고,
	 * BookDao를 상속받아 BookService 를 만들었다
	 * 이러한 다중상속관계에서
	 * BookDao 를 사용하여 서비스를 구현하려고 한다
	 * 
	 * 보통 Dao를 설정할때 private final 로 선언하고
	 * 생성자에서 주입받아 사용했다
	 * 
	 * 그런데 다중상속관계에서
	...
	/*
	 * Spring 에서 어던 자동화된 코드를 실행하고 싶을때
	 *  method 를 만들고 method에 @Autowired를 설정해 두면
	 *  서비스가 처음 실행될때 
	 */
	@Autowired
	@Override
	public void creat_book_table() {
		...
		try {
			 ...
			 * Service 클래스에 Autowirde 된 method 를 선언하고
			 * 자동으로 실행되도록 한다
			 */
			bookDao.creat_book_table();
	...
	public int insert(BookVO vo) {
		 ...
		 * INSERT 된 데이터의 행 개수를 return한다
		 * 만약 return 된 값이 0이라면 한개의 데이터도 INSERT 하지 못한 것이다
		 * 1이상의 정수 값이 return 되면 몇개의 데이터가 INSERT되었는지
		 * 확인할 수 있다
		 */
