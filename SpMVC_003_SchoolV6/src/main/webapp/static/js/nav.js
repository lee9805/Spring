document.addEventListener("DOMContentLoaded", () => {
  ...
  if (nav) {
    nav.addEventListener("click", (e) => {
      /*
         e.target
         현재 nav > ul > li 형태로 layout구성되어 있다
         이 layout 의 제일 바깥쪽인 nav tag에 click event 를 설정했다
         마우스로 nav tag 의 어느곳을 클릭하면
         event 가 nav > ul > li 로 점점 전파된다 : event Bubbling 이라고 하낟
          event Bubbling은 상당히 불편한 존재이나
          이 현상을 역으로 이용하여 실제 li 에 click이 되었음을 감지한다
