document.addEventListener("DOMContentLoaded", () => {
  ...
  if (std_save) {
    std_save.addEventListener("click", () => {
      ...
      //JS 코드를 사용하여 Clien Size Validation 을 수행
      // JS 코드를 사용하여 Front  단에서 유효성 검사
      //값을 입력하지 않았을때
      //경고 메시지를 보여주고 유효성 검사 중단
      if (st_num.value === "") {
        alert("학번은 반드시 입력해야한다");
        st_num.focus();
        return false;
      }
      ...
      Java에서는 NumberFormatException 이 발생하고 코드가 중단된다
      코드 중단을 방지하기 위햐여 try-catch로 처리를 해주어야 한다

      그런데
      JS에서는 그러한 상황에 Exception 이 발생하는 대신
