/*
 비동기 방식의 코드문제로
 return falsse 또는 return true 가 실행되지 않는 문제를 해결하기 위하여
...
const st_num_fetch = (st_num) => {
  ...
  //fetch 가 완료되기 전에 실행되어 st_num_fetch가 종료됨
  //이 함수가 true 나 false를 retrue 할것으로 기대하고
  //작성을 했으나 비동기 방식의 이유로
...
const save_cb = async () => {
  ...
  //st_num_yes 에 true 나 false 가 담길것으로 기대한고 있다
  //false 가 담기면 학번 항목을 clear하고 focus() 를 담는다
  //중복된 학번일 경우가 해당된다
