...
public class UserController {
	...
	/*
	 * UserVO 를 ModelAttribute로 생성하여 join.jsp에게 보내기
	 * USerVO userVO = new UserVO(); 코드를 대신 실행해준다
	 * 
	 * ModelAttribute 로 생성된 userVO 는
	 * SessionAttribute 에 추가되어 보관된다
	 */
	...
	public String join(@Valid UserVO userVO, BindingResult result) {
		...
		//인증메일을 보내고 나면 userVO 의 key_ok변수에는
		//인증 키가 담겨있는 상태가 된다
	 ...
	 * 데이터가 있으면 그냥 그대로 통과를 하고
	 * 만약 데이터가 없으면 SessionAttribute 에 보관중인 userVo데이터를 가져와서
	 * 매개변수 userVO 에 세팅한다
	 */
	...
	public String join_next(@Valid @ModelAttribute("userVO")UserVO userVO,
