...
public class HomeController {
	...
	public String home(@ModelAttribute("emsVO") EmsVO emsVO, Locale locale, Model model) {
		...
		if(matcher.find()) {
			...
			log.debug("Naveruse{}", user);
		}

		log.debug("naver_username{}",naver_username);
		log.debug("앱 이름{}",myApp);
		model.addAttribute("emsVO",emsVO);
		return "home";
	}
	/*
	 * @Valid : vo(dto)  클랫에 유효성 검사 코드를 추가해 두고
	 * form 으로 부터 데이터가 전송되어 오면 자체적으로 검사를 수행한다
	 * 
	 * 만약 vo 에  설정된 유효성 검사가 한개라도 통과하지 못하면
	 * BindingReseult 객체에 오류메시지가 포함된다
	 * BindingReseult 객체의 hasErrors() method 를 사용하여
	 * 오류가 있는지 확인하고
