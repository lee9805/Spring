...
/*
 * search 문자열이 null일 경우 전체 데이터를 조회할 수 있도록 만드는 코드를
 * Controller 에서 Service로 이동
 */
	@Override
	...
	public void searchAndPage(Model model, SearchPage searchPage) {
		...
		//검색어가 없이 요청이 될경우 SearchPage 객체의 search 값이 null이 되어
		//데이터가 검색이 되지 않는다
		...
		// 		만약 전체 데이터가 37개이고 한 페이지에 10개를 보여준다
		...
		// 선택된 페이지 번호에 따라 화면 하단 번호 리스트를 유동적으로
		// 보여주기 위한 연산
