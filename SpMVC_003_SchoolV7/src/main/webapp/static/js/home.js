document.addEventListener("DOMContentLoaded", () => {
  ...
  화면에 li tag 가 여러개가 있다 여러개의 li tag 에 click event를 설정하려면
  document, querSelectorAll() 함수를 사용하여 전체 tag list를 배열로 받고
  for() 반복문을 사용하여 각가가의 요소에 event 을 부여하는 코드가 필요하다

  지금은 li tag 를 감싸고 있는 nav tag 한개에만 click event 를 적용하여
  event Bubbling 을 역으로 활용하여 사용한다
  nav.main_nav에 click event 를 설정했다면
  e 매개변수의 target 속성을 사용하면 가장 안쪽의 li tag에 접근이 가능하다
  li tag click 되면 tagname 을 조회보고 실제로 Li tag 가 클리된 것이면
  Li tag 의 text 을 가져와서 Location  을 변경하는 용도로 활용한다
