...
<body>
	<section class="w3-container">
		<div class="w3-container detail-flex">
			...
			<div class="detail">
				<h2>도서명:<a href="${BOOK.link}" target=_NEW>${BOOK.title}</a></h2>
				<p>ISBN : ${BOOK.isbn}
				<p>저자: ${BOOK.author}
		...
		<div class="btn-box">
			<a href="${rootPath}/books/list" class="list">리스트로 가기</a> <a
...
<script>
	...
	.replace() 는 현재 화면의 history 를 제거해 버리고 새로운 page 로 
	덮어쓰기가 된다
	뒤로가기를 눌렀을때 현재 보고있는 페이지를 열수 없다는 것이다
	
	보통 Datail에서 삭제를 선택했을때
	삭제하고난 후 뒤로가기를 눌러 이미 삭제된 데이터를 보는 석을
	방지하기 위함이다
	*/
</script>
</html>