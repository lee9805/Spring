...
/*
 * Spring Security에서 로그인 사용자 정보를 dB로 부터
 * 가져와서 핸들링 하는 클래스
...
public class UserDetailsServiceImpl implements UserDetailsService{
	 ...
	 * UserDao를 통해 로그인한 사용자 정보를 DB로 부터 가져온다
	 */
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		//로그인한 사용자의 username 이 table에 없으면
		if(userVO == null) {
			// exception 을 강제로 발생시키기
			//Spring Security 에게 exception 발생시켜
			// 메시지 전달하기
			throw new UsernameNotFoundException(username +  ": 회원가입을 먼저 하세요");
		}
		//username 에 해닿ㅇ하는 Role 정보를 tbl_authorities table에서 가져오기
		...
		// Grant List USerVO.authorities 에 저장
